...
import com.todoist.repository.SectionRepository;
...
public class TaskService {
    ...
    /**
     * Duplicate a task: a deep copy of the task AND all its sub-tasks (same name,
     * same project/section/parent, labels + assignee carried over; comments are
     * NOT copied). The copy is inserted right after the original among its
     * siblings. The whole sub-tree starts uncompleted.
     */
    @Transactional
    public TaskDto duplicate(UUID taskId, UUID userId) {
        Task original = loadOwnedTask(taskId, userId);
        UUID projectId = original.getProject().getId();

        // Make room directly below the original: shift later siblings down by one.
        UUID parentId = parentIdOf(original);
        UUID sectionId = sectionIdOf(original);
        for (Task sib : taskRepository.findByProjectId(projectId)) {
            if (!sib.getId().equals(original.getId())
                    && java.util.Objects.equals(parentIdOf(sib), parentId)
                    && java.util.Objects.equals(sectionIdOf(sib), sectionId)
                    && sib.getPosition() > original.getPosition()) {
                sib.setPosition(sib.getPosition() + 1);
            }
        }

        User creator = userRepository.getReferenceById(userId);
        Task copy = copyTaskTree(original, original.getParentTask(), creator);
        copy.setPosition(original.getPosition() + 1);

        realtime.publish(projectId);
        return TaskDto.from(copy);
    }

    /** Recursively copy a task and its sub-tasks under {@code newParent}. */
    private Task copyTaskTree(Task src, Task newParent, User creator) {
        Task copy = new Task();
        copy.setProject(src.getProject());
        copy.setParentTask(newParent);
        ...
        copy.setDueTime(src.getDueTime());
        copy.setPosition(src.getPosition());
        copy.setAssignee(src.getAssignee());
        copy.getLabels().addAll(src.getLabels());
        Task saved = taskRepository.save(copy);

        for (Task child : taskRepository.findByParentTaskIdOrderByPosition(src.getId())) {
            copyTaskTree(child, saved, creator);
        }
        return saved;
    }
