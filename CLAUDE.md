...
- **Task-detail modal + Popover + DatePicker rework. ✅ confirmed working 2026-06-20:**
  ...
  - **Popover (`components/Popover.jsx`):** renders in a **portal** (`createPortal`, `position:fixed`, clamps to viewport so tall popups shift up) → escapes the modal's `overflow` (fixed the clipped date box). `fullWidth` prop; `.no-scrollbar` util added to `index.css`.
